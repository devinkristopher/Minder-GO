...
final class AppViewModel: ObservableObject {
    ...
    var headerText: String {
        if (scanType.rawValue == "barcode") {return "scan"}
        else { return "\(scanType.rawValue)" }
        
    }
    
    var statusText: String {
        // If app is in barcode mode
        if (scanType.rawValue == "barcode") {
            if (recognizedItems.isEmpty) {
                return("Scanning for miners...")
            }
            else {
